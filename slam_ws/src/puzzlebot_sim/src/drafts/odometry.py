...
class MyOdometryPublisher():
    def __init__(self, variance_angvel = 0.01):
        ...
        self.pJS = rospy.Publisher('/joint_states', JointState, queue_size=1)
        self.js = JointState()
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            qRota = tf.transformations.quaternion_from_euler(0, 0, self.model_state[0])
            cTime = rospy.Time.now()

            # Transform
            self.tb.sendTransform([self.model_state[1], self.model_state[2], 0], qRota, cTime, "base_link", "map")
